...
function generate(url, filename, req, res, next) {
    if (!url) {
    ...
    function generatePdf() {
        ...
        var tmpDataFile;
        if (url.length > 10000) {
            tmpDataFile = path.join(__dirname, '../../tmp', tmpId + '.data');
            console.info('URL exceeds 10000 characters, writing to temp file:', tmpDataFile);
            fs.writeFileSync(tmpDataFile, url);
            url = '--file=' + tmpDataFile;
        }

        var tmpFile = path.join(__dirname, '../../tmp', tmpId + '.pdf');
        var outputLog = '';
...
module.exports = function (app) {
    var textParser = bodyParser.text({ type: 'text/plain', limit: '10mb' });
    app.post('/', textParser, function (req, res, next) {
        var url = req.body;
        var filename = 'GeneratedPDF';
        ...
        generate(url, filename, req, res, next);
    });
