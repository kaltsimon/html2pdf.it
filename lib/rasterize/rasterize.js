...
if (system.args.length < 3 || system.args.length > 7) {
	...
	match = address.match(/^--file=(.*)/);
	if (match && match[1]) {
		console.info('Trying to load file:', match[1]);
		try {
			var file = fs.open(match[1], { mode: 'r', charset: 'utf-8' });
			address = file.read();
			console.info('File loaded.');
			console.info(address.slice(0, 100));
		} catch (e) {
